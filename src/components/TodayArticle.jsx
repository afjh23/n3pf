...
export default function TodayArticle() {
  ...
  return (
    ...
    xsm:w-[459px] xsm:h-full xsm:shrink-1 relative' >
      ...
      {formattedData && formattedData.length > 0 && (
        <>
          <div className='h-[40px] w-full flex justify-between px-[11px] text-white font-PrincipalFont'>
            <button className='w-[161px] h-[40px] bg-[#6e707a]' onClick={toggleModal}>Search for places</button>
            <button className='w-[40px] h-[40px] flex justify-center items-center bg-[#6e707a] rounded-full' onClick={() => navigator.geolocation.watchPosition(success, error, options)}>
          ...
          <div className='flex flex-col items-center font-PrincipalFont text-[#a09fb1] max-h-[359px] h-full place-content-between xsm:max-h-[425px]'>
            <div>
              ...
              <span className='text-[48px] font-medium'>{unit==='metric'? '°C' : '°F'}</span>
            </div>
